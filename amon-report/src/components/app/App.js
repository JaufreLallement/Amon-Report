...
// Component imports
import { FileUpload, Button, Alert, Spin, Report, Flex, Fieldset, Icon, BannerPortal } from '../';

// Script imports
import { Files, Strings } from '../../scripts';
import { Configuration } from '../report/configuration';
...
const App = () => {
  ...
  const getReport = (data, config) => {
    const conf = new Configuration({ ...config, [attrStr]: { version, ...config[attrStr] } }, data);
  ...
  return (
    <Col className="App" alignY="start">
      {content}
